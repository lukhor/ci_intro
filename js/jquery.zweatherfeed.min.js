		...
		return this.each(function(q, t) {
				...
				success : function(f) {
					if (f.query) {
						if (0 < f.query.results.channel.length)
							...
							w(t, f.query.results.channel, h);
						j.isFunction(v) && v.call(this, l)
					} else
						h.showerror && l.html("<p>Služba je momentálne nedostupná</p>")
				},
				error : function() {
					h.showerror && l.html("<p>Požiadavok je nedostupný</p>")
				}
			});
			var w = function(f, c, e) {
				...
				if ("Yahoo! Weather Error" != c.description) {
					...
					e.highlow && (b += '<div class="weatherRange">Teplota Max: ' + g.high + "&deg; Min: " + g.low + "&deg;</div>");
					e.wind && (b += '<div class="weatherWind">Vietor: ' + a + " " + c.wind.speed + c.units.speed + "</div>");
